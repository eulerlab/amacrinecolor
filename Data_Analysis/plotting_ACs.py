...
def plot_Kernels(dataframe, clustering_name,  nb_clusters = None, savefig = False, path_ROIs = None, path_means = None):
    ...
    plt.xticks(np.array([(size_kernel/2)+i*size_kernel for i in range(6)]) - 0.5, labels)
    ...
    cluster_means = np.zeros((nb_clusters,size_kernel*6))
    ...
    fig, ax = plt.subplots(nb_clusters, 6, sharex='all', sharey='all', figsize=(4,8))
    for current_cluster_ID in range(nb_clusters):
        for current_kernel_ID in range(6):
